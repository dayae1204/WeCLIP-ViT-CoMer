...
class SegFormerHead(nn.Module):
    ...
    - 수정: 단일 텐서(final_cti)를 처리하도록 간소화
    """
    def __init__(self, in_channels=128, embedding_dim=256, num_classes=20, index=11, **kwargs):
        ...
        # 중요 변경: 기존의 MLP 모듈과 다른 복잡한 처리 제거
        # 오직 linear_fuse(1x1 conv)만 유지하여 채널 수 변환


        # 중요 변경: 단일 768채널 텐서를 256채널로 변환하는 1x1 conv만 유지
        self.linear_fuse = nn.Conv2d(768, embedding_dim, kernel_size=1)
        self.dropout = nn.Dropout2d(0.1)
        
    def forward(self, x):
        """
        x: 단일 텐서 final_cti [N, 768, H/16, W/16]
