...
def deform_inputs_only_one(query_tensor, h, w):
    ...
    반환:
    - reference_points: query_tensor와 동일한 sequence length를 가지는 참조 포인트
    - spatial_shapes: 입력 특징 맵의 공간 크기
    ...
    print(f"Warning: query sequence length ({seq_len}) doesn't match total length from spatial shapes (537600)")
    
    # reference_points 생성
    if has_class_token:
        # Class token + 나머지 토큰
        # Class token을 위한 별도 레벨과 grid_size x grid_size 레벨
        spatial_shapes = torch.tensor([[1, 1], [grid_size, grid_size]], 
                                     dtype=torch.long, device=query_tensor.device)
        level_start_index = torch.tensor([0, 1], dtype=torch.long, device=query_tensor.device)
        
        # Class token을 위한 dummy reference point
        dummy_ref = torch.zeros((1, 1, 1, 2), dtype=torch.float32, device=query_tensor.device)
        
        # 나머지 토큰을 위한 reference points
        grid_ref = get_reference_points([(grid_size, grid_size)], device=query_tensor.device)
        
        # 합치기
        reference_points = torch.cat([dummy_ref, grid_ref], dim=1)  # [1, 1+grid_size^2, 1, 2]
    else:
        # Class token이 없는 경우
        spatial_shapes = torch.tensor([[grid_size, grid_size]], 
                                     dtype=torch.long, device=query_tensor.device)
        level_start_index = torch.zeros(1, dtype=torch.long, device=query_tensor.device)
        
        # reference points 생성
        reference_points = get_reference_points([(grid_size, grid_size)], device=query_tensor.device)
