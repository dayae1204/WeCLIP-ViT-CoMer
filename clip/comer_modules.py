...
def deform_inputs_only_one(x, h, w):
    # 수정된 함수 - 데이터 타입 일관성 확보 및 크기 검증 추가
    # h, w는 원본 이미지 크기, 실제 특징 맵 크기는 h/16, w/16 등으로 계산
    
    # 입력 검증
    assert h % 32 == 0 and w % 32 == 0, f"Image size must be divisible by 32, got {h}x{w}"
    ...
    # 각 레벨의 특징 맵 크기가 유효한지 확인
    assert h_8 > 0 and w_8 > 0, f"Feature map size at level 1 is too small: {h_8}x{w_8}"
    assert h_16 > 0 and w_16 > 0, f"Feature map size at level 2 is too small: {h_16}x{w_16}"
    assert h_32 > 0 and w_32 > 0, f"Feature map size at level 3 is too small: {h_32}x{w_32}"
    
    # spatial_shapes 생성
    spatial_shapes = torch.as_tensor([(h_8, w_8), (h_16, w_16), (h_32, w_32)],
                                    dtype=torch.long, device=x.device)
    
    # level_start_index 계산
    level_start_index = torch.cat((spatial_shapes.new_zeros(
        (1,)), spatial_shapes.prod(1).cumsum(0)[:-1]))
    
    # reference_points 생성
    reference_points = get_reference_points([(h_8, w_8), (h_16, w_16), (h_32, w_32)], 
                                           device=x.device)
    
    # 특징 맵 크기와 reference_points 크기 검증
    total_points = sum([h*w for h, w in zip(spatial_shapes[:, 0].tolist(), 
                                          spatial_shapes[:, 1].tolist())])
    assert reference_points.shape[1] == total_points, \
        f"Reference points count ({reference_points.shape[1]}) doesn't match total points from spatial shapes ({total_points})"
