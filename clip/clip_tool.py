...
def generate_clip_fts(image, model, require_all_fts=True):
    ...
    if require_all_fts:
        # ViT-Comer 모드
        transformer_features, attn_weights = model.encode_image(image, h, w, require_all_fts=True)
        
        # transformer_features: 각 transformer block의 feature maps (LND 형태)
        # attn_weights: 각 transformer block의 attention weights (batch × seq_len × seq_len)
        
        # WeCLIP의 기존 코드와 호환되도록 반환
    ...
    for idx, label in enumerate(label_list):
        ...
        if idx == 0:
            if require_seg_trans == True:
                ...
                attn_weight = attn_weight[:, 1:, 1:][-8:]  # 마지막 8개 어텐션 맵
