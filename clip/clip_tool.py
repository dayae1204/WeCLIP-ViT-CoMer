...
def generate_clip_fts(image, model, require_all_fts=True):
    ...
    if require_all_fts:
        # ViT-Comer 모드
        vit_features, cti_features = model.encode_image(image, h, w, require_all_fts=True)
        
        # vit_features는 마지막 transformer block의 출력
        # cti_features는 CTI 모듈의 최종 출력 (8개 CTI 출력의 concat + 1x1 conv)
        
        # WeCLIP의 기존 코드와 호환되도록 반환 형식 유지
        # vit_features를 fts_all로 사용하고, cti_features를 attn_weight_list로 사용
        return vit_features, cti_features
    else:
        # 기존 방식
        image_features_all, attn_weight_list = model.encode_image(image, h, w, require_all_fts=False)
        return image_features_all, attn_weight_list
...
def perform_single_voc_cam(img_path, image, image_features, attn_weight_list, seg_attn, bg_text_features,
    ...
    for idx, label in enumerate(label_list):
        ...
        if idx == 0:
            ...
            # ViT-Comer에서는 attn_weight_list가 CTI 출력이므로 이를 적절히 처리
            if require_seg_trans == True:
                # CTI 출력을 attention weight로 사용
                attn_weight = attn_weight_list.flatten(2, 3)  # NCHW -> NC(HW)
                
                # attn_weight 형태 변환 (필요에 따라 수정)
                if len(attn_weight.shape) == 2:  # NC 형태인 경우
                    attn_weight = attn_weight.reshape(attn_weight.shape[0], h//16, w//16, h//16, w//16)
                    attn_weight = attn_weight.permute(0, 1, 3, 2, 4).reshape(attn_weight.shape[0], (h//16)*(w//16), (h//16)*(w//16))
                ...
                if isinstance(attn_weight, list):
                    attn_weight = torch.stack(attn_weight, dim=0)
                attn_weight = attn_weight[:, 1:, 1:][-8:]
                attn_weight = torch.mean(attn_weight, dim=0)  # (hw, hw)
                attn_weight = attn_weight.detach()
