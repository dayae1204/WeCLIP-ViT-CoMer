...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, H, W, require_all_fts=False):
        ...
        for stage_idx, (start_block, end_block) in enumerate(self.stage_indices):
            ...
            # VisionTransformer의 forward 메서드에서 MRFP 출력의 시퀀스 길이 조정
            processed_cnn_shape = processed_cnn.shape
            if processed_cnn.dim() == 3 and processed_cnn.shape[1] != adapter_v_output.shape[0]:
                print(f"Reshaping processed_cnn from {processed_cnn.shape} to match adapter_v_output")
                
                # processed_cnn를 adapter_v_output과 일치하는 시퀀스 길이로 조정
                if adapter_v_output.dim() == 3 and adapter_v_output.shape[0] > adapter_v_output.shape[1]:
                    # adapter_v_output이 LND 형식이고, processed_cnn이 NLD 형식인 경우
                    target_len = adapter_v_output.shape[0]
                    N, L, D = processed_cnn.shape
                    
                    if L > target_len:
                        # 크기 줄이기 (첫 target_len 개만 사용)
                        processed_cnn = processed_cnn[:, :target_len, :]
                    elif L < target_len:
                        # 크기 늘리기 (패딩)
                        padding = torch.zeros((N, target_len - L, D), dtype=processed_cnn.dtype, device=processed_cnn.device)
                        processed_cnn = torch.cat([processed_cnn, padding], dim=1)
            ...
            # 디버깅 출력
            print(f"adapter_v_output shape: {adapter_v_output.shape}")
            print(f"processed_cnn shape: {processed_cnn.shape}")
            ...
            try:
                ...
                try:
                    ...
                    fallback_output = self.cti_to_v_modules[stage_idx]._fallback_attention(
                        adapter_v_output, processed_cnn
                    )
                    ...
                    cti_outputs.append(adapter_v_output)
