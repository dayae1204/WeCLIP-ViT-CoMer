...
class WeCLIP(nn.Module):
    ...
    def forward(self, img, img_names, mode='train'):
        ...
        #        
        # 중요 변경: final_cti를 decoder에 직접 전달
        # final_cti는 이미 model.py에서 8개의 CTI 출력을 concat하고 1x1 conv를 적용한 결과
        
        # 중요: final_cti를 attn_fts로 사용
        attn_fts = final_cti
        
        # decoder를 통해 segmentation 결과 생성
        seg, seg_attn_weight_list = self.decoder(final_cti)
        
        # affinity map 생성 - final_cti(attn_fts)를 사용
